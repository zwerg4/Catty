...
final class SetLookByIndexBrickTest: XCTestCase {
    ...
    func testSetLookByValidIndex() {
        ...
        action()
        XCTAssertEqual(spriteNode.currentLook, look3, "setLookByIndexBrick not correct")

        brick.lookIndex = Formula(integer: 2)
        action = brick.actionBlock(formulaInterpreter)
        action()
        XCTAssertEqual(spriteNode.currentLook, look2, "setLookByIndexBrick not correct")

        brick.lookIndex = Formula(integer: 1)
        action = brick.actionBlock(formulaInterpreter)
        action()
        XCTAssertEqual(spriteNode.currentLook, look, "setLookByIndexBrick not correct")
    }
